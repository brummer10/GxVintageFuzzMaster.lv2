...
static void resize_event(gx_vfmUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_vfmUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_vfmUI* ui = (gx_vfmUI*)malloc(sizeof(gx_vfmUI));
	...
	ui->controls[0] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 30, 30, 71, 71}, false,"INTENSITY", KNOB, INTENSITY};
	ui->controls[1] = (gx_controller) {{ 0.0, 0.0, 0.0, 1.0, 1.0}, { 125, 30, 71, 71}, false,"MODE", SWITCH, MODE};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 220, 30, 71, 71}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
	...
	ui->width = ui->init_width -140 + (80 * CONTROLS);
	...
	ui->frame = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, 71, 91);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-140 + (80 * CONTROLS));
...
static void knob_expose(gx_vfmUI *ui,gx_controller* knob) {
	...
	if (knob->type == KNOB && knob->is_active == true) {
		...
		cairo_move_to (ui->crf, knobx1-knob_x/2.4-extents.width/1.6, knoby1+knob_y/2+extents.height/1.1);
		cairo_show_text(ui->crf, "Low");
		cairo_new_path (ui->crf);

		cairo_text_extents(ui->crf,"High", &extents);
		cairo_move_to (ui->crf, knobx1+knob_x/2.6-extents.width/2.3, knoby1+knob_y/2+extents.height/1.4);
		cairo_show_text(ui->crf, "High");
		cairo_new_path (ui->crf);
	}
...
static void bypass_expose(gx_vfmUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_vfmUI *ui, gx_controller* controller) {
...
static void _expose(gx_vfmUI *ui) {
	static const char* plug_name = "GxVintageFuzzMaster" ;
...
static void controller_expose(gx_vfmUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_vfmUI *ui, int controller) {
...
static void check_value_changed(gx_vfmUI *ui, int i, float* value) {
...
static void check_is_active(gx_vfmUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_vfmUI *ui) {
...
static bool get_active_ctl_num(gx_vfmUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_vfmUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_vfmUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_vfmUI *ui, int i) {
...
static void button1_event(gx_vfmUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_vfmUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_vfmUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_vfmUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_vfmUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_vfmUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_vfmUI *ui, bool set) {
...
static void event_handler(gx_vfmUI *ui) {
	XEvent xev;
