...
#define GTK_VINTAGE_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_VINTAGE_KNOB, GtkVintageKnobPrivate))

G_DEFINE_TYPE(GtkVintageKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkVintageKnobPrivate GtkVintageKnobPrivate;

struct _GtkVintageKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	/** check resize **/
	int grow;
 	GtkVintageKnobPrivate *priv = GTK_VINTAGE_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model) {
        ...
        cairo_move_to (cr, knobx1-knob_x/2.4-extents.width/1.8, knoby1+knob_y/2+extents.height);
...
{
	g_assert(GTK_IS_VINTAGE_KNOB(widget));
	GtkVintageKnob *knob = GTK_VINTAGE_KNOB(widget);
	GtkVintageKnobPrivate *priv = GTK_VINTAGE_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkVintageKnobClass
 */

static void gtk_knob_class_init (GtkVintageKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkVintageKnobPrivate));
}
...
static void gtk_knob_init (GtkVintageKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_VINTAGE_KNOB, NULL ));
